# ICAA17K DAT 3번 TransformerStage의 PyTorch-TensorFlow 동등성을 검증하는 스크립트
...
REPORT_REL_PATH = "experiments/icaa_tf_native/reports/stage_g3_transformer_stage3_parity.md"
OUTPUT_PREFIX = "icaa_tf_native_stage_g3"
COMMAND_USED = "python experiments/icaa_tf_native/scripts/stage_g3_transformer_stage3_parity.py"
...
def import_dat_from_package_dir(package_dir: Path):
    package_name = "icaa_export_safe_v3_export_safe_models_stage_g3"
...
def likely_failed_subcomponent(row: dict | None) -> str | None:
    ...
    if "_attn_out" in name:
        ...
        attn_type = "D" if block_idx == 1 else "L"
        return f"block {block_idx} attention type {attn_type}"
    if "_after_attn_residual" in name:
        return f"{name.split('_')[0]} attention residual, layout, or drop path"
    if "_mlp_out" in name:
        return f"{name.split('_')[0]} MLP"
    if "_output" in name:
        return f"{name.split('_')[0]} MLP residual, layout, or drop path"
...
def write_markdown_report(report: dict, report_path: Path) -> None:
    ...
    body = f"""# Stage G-3 TransformerStage 3 Parity
...
- Safe to proceed to full TensorFlow DAT assembly parity: {report['safe_to_proceed_to_full_tf_dat_assembly_parity']}
...
def run() -> tuple[dict, dict]:
    ...
    try:
        with warnings.catch_warnings(record=True) as captured:
            ...
            checkpoint_tensors_used = [name for name in state_dict.keys() if name.startswith("stages.3.")]
            ...
            pytorch_stage_inspection = {
                ...
                "stage_index": 3,
                "stage_input_shape_nchw": [None, 1024, 7, 7],
                "stage_output_shape_nchw": [None, 1024, 7, 7],
                "depth": int(stage.depths),
                "stage_spec": MODEL_KWARGS["stage_spec"][3],
                "proj_class": stage.proj.__class__.__name__,
                "attention_blocks": [
                    {
                        "index": idx,
                        "class_name": attn.__class__.__name__,
                        "attention_type": MODEL_KWARGS["stage_spec"][3][idx],
        ...
        overall_pass = bool(all(result["pass"] for result in results))
        ...
        debug_summary = {
            ...
            "captured_random_image_stage3_input": {
                "input": tensor_summary("captured_random_image_stage3_input_nchw", random_captured_input),
                "output_result": captured_result,
                "per_block_diffs": captured_debug,
                "dattention_position_reference_diffs": captured_dattention,
            },
            "captured_real_image_stage3_input": None,
            "real_image_paths": real_image_paths,
        }
        if real_result is not None:
            debug_summary["captured_real_image_stage3_input"] = {
                "input": tensor_summary("captured_real_image_stage3_input_nchw", real_captured_input),
        ...
        report = {
            ...
            "test_input_generation": {
                ...
                "captured_random_image_stage3_input": {
                    "distribution": "numpy randn image passed through PyTorch patch_proj, stages 0/1/2, and down_projs 0/1/2",
                    "seed": SEED,
                },
                "captured_real_image_stage3_input": {
                    "source": CSV_REL_PATH,
                    "preprocessing": "PIL bilinear resize to 224x224, RGB [0,1], ImageNet mean/std normalization, then PyTorch patch_proj, stages 0/1/2, and down_projs 0/1/2",
                },
            },
            ...
            "real_image_paths": real_image_paths,
            "safe_to_proceed_to_full_tf_dat_assembly_parity": overall_pass,
        ...
        report = {
            ...
            "real_image_paths": [],
            "safe_to_proceed_to_full_tf_dat_assembly_parity": False,
            "unresolved_issues": ["Stage G-3 script failed before completing TransformerStage 3 parity."],
            "warnings": warnings_seen,
            "errors": errors,
            "log_lines": log_lines,
        }
...
def main() -> int:
    ...
    json_path = output_dir / "stage_g3_transformer_stage3_report.json"
    log_path = output_dir / "stage_g3_transformer_stage3_log.txt"
    ...
    if report["status"] in {"ok", "parity_failed"}:
        ...
        markdown_path.write_text(
            ...
            "- Stage G-3 failed before TransformerStage 3 parity completed. See JSON and log artifacts for details.\n",
            encoding="utf-8",
        )
