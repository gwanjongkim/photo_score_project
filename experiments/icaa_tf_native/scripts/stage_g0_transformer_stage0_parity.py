# ICAA17K DAT 0번 TransformerStage의 PyTorch-TensorFlow 동등성을 검증하는 스크립트
...
REPORT_REL_PATH = "experiments/icaa_tf_native/reports/stage_g0_transformer_stage0_parity.md"
OUTPUT_PREFIX = "icaa_tf_native_stage_g0"
COMMAND_USED = "python experiments/icaa_tf_native/scripts/stage_g0_transformer_stage0_parity.py"
...
def import_dat_from_package_dir(package_dir: Path):
    package_name = "icaa_export_safe_v3_export_safe_models_stage_g0"
...
def import_tf_stage0(root: Path):
    sys.path.insert(0, str(root / "experiments" / "icaa_tf_native"))
    from tf_models.transformer_stage import TFTransformerStage0

    return TFTransformerStage0
...
def stage0_torch_with_debug(torch, stage, stage_input):
    debug = {}
    with torch.no_grad():
...
def run_case(torch, tf, np, stage_pt, stage_tf, name: str, stage_input_nchw):
    ...
    y_pt, positions, references, debug_pt = stage0_torch_with_debug(torch, stage_pt, x_pt)
...
def patch_projection_output(torch, model, image_nchw):
    with torch.no_grad():
        return tensor_np(model.patch_proj(torch.from_numpy(image_nchw.astype("float32"))))
...
def write_markdown_report(report: dict, report_path: Path) -> None:
    ...
    body = f"""# Stage G-0 TransformerStage 0 Parity
...
- Shift-window attention mask: direct copy from `stages.0.attns.1.attn_mask`.
...
## Stage G-1 TransformerStage 1 Decision
- Safe to proceed to Stage G-1 TransformerStage 1 parity: {report['safe_to_proceed_to_stage_g1_transformerstage1_parity']}
- Scope of this decision: `TransformerStage` stage `0` only. This does not establish full TensorFlow DAT feasibility.
...
def run() -> tuple[dict, dict]:
    ...
    try:
        with warnings.catch_warnings(record=True) as captured:
            ...
            checkpoint_tensors_used = [name for name in state_dict.keys() if name.startswith("stages.0.")]
            ...
            TFStage0 = import_tf_stage0(root)
            stage_tf = TFStage0(dim=128, heads=4, window_size=7, expansion=4, epsilon=1e-5)
            stage_tf.build_for_input_shape((1, 56, 56, 128))
            stage_tf.load_from_pytorch_state_dict(state_dict, prefix="stages.0")

            pytorch_stage_inspection = {
                "class_name": stage.__class__.__name__,
                "stage_index": 0,
                "stage_input_shape_nchw": [None, 128, 56, 56],
                "stage_output_shape_nchw": [None, 128, 56, 56],
                "depth": int(stage.depths),
                "stage_spec": MODEL_KWARGS["stage_spec"][0],
                ...
                "drop_path_eval_behavior": "All stage-0 drop_path modules are deterministic identity in eval mode; block 0 is nn.Identity and block 1 DropPath is disabled by eval().",
                "residual_order": [
                    "x = proj(x)",
                    "x0 = x",
                ...
                "down_projection_relationship": {
                    "is_part_of_stage0": False,
                    "dat_forward_order": "DAT.forward calls model.stages[0](x), then applies model.down_projs[0](x) outside the stage when i < 3.",
                    "down_proj0_class": model.down_projs[0].__class__.__name__,
                },
            }

            random_stage_input = np.random.randn(1, 128, 56, 56).astype(np.float32)
            random_result, random_debug = run_case(
                torch, tf, np, stage, stage_tf, "deterministic random stage-0 input", random_stage_input
            )

            random_image = np.random.randn(1, 3, 224, 224).astype(np.float32)
            random_patch_input = patch_projection_output(torch, model, random_image)
            patch_result, patch_debug = run_case(
                torch,
                tf,
                np,
                stage,
                stage_tf,
                "patch projection output from deterministic random image",
                random_patch_input,
            )
            ...
            real_result = None
            real_debug = None
            real_patch_input = None
            if real_image_nchw is not None:
                real_patch_input = patch_projection_output(torch, model, real_image_nchw)
                real_result, real_debug = run_case(
                    ...
                    stage,
                    stage_tf,
                    "patch projection output from 3 real ICAA17K images",
                    real_patch_input,
                )

            results = [random_result, patch_result]
        ...
        safe_to_stage_g1 = bool(all(result["pass"] for result in results))
        ...
        debug_summary = {
            ...
            "random_image_patch_projection": {
                "input": tensor_summary("random_patch_projection_output_nchw", random_patch_input),
                "output_result": patch_result,
                "subcomponent_diffs": patch_debug,
            },
            "real_image_patch_projection": None,
            "real_image_paths": real_image_paths,
        }
        if real_result is not None:
            debug_summary["real_image_patch_projection"] = {
                "input": tensor_summary("real_patch_projection_output_nchw", real_patch_input),
                "output_result": real_result,
                "subcomponent_diffs": real_debug,
            }

        report = {
            "status": "ok" if safe_to_stage_g1 else "parity_failed",
            "overall_pass": safe_to_stage_g1,
            ...
            "test_input_generation": {
                ...
                "random_image_patch_projection": {
                    "distribution": "numpy randn image passed through PyTorch patch_proj",
                    "seed": SEED,
                },
                "real_image_patch_projection": {
                    "source": CSV_REL_PATH,
                    "preprocessing": "PIL bilinear resize to 224x224, RGB [0,1], ImageNet mean/std normalization, then PyTorch patch_proj",
                },
            },
            ...
            "real_image_paths": real_image_paths,
            "safe_to_proceed_to_stage_g1_transformerstage1_parity": safe_to_stage_g1,
        ...
        report = {
            ...
            "real_image_paths": [],
            "safe_to_proceed_to_stage_g1_transformerstage1_parity": False,
            "unresolved_issues": ["Stage G-0 script failed before completing TransformerStage 0 parity."],
            "warnings": warnings_seen,
            "errors": errors,
            "log_lines": log_lines,
        }
...
def main() -> int:
    ...
    json_path = output_dir / "stage_g0_transformer_stage0_report.json"
    log_path = output_dir / "stage_g0_transformer_stage0_log.txt"
    ...
    if report["status"] in {"ok", "parity_failed"}:
        ...
        markdown_path.write_text(
            "# Stage G-0 TransformerStage 0 Parity\n\n"
            f"- Status: `{report['status']}`\n"
            f"- Output directory: `{report['output_dir']}`\n"
            "- Stage G-0 failed before TransformerStage 0 parity completed. See JSON and log artifacts for details.\n",
            encoding="utf-8",
        )
