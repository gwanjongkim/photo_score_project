# RGNet 논문 지향 AVA 이진 분류 실험을 학습한다.
...
from src.models.rgnet_paper_ava import (
    MODEL_VARIANT,
    RAW_THEN_SIGMOID,
    build_rgnet_paper_ava_model,
    get_rgnet_paper_ava_custom_objects,
)


DEFAULT_OUTPUT_DIR = "outputs/rgnet_paper_ava_classification_20260510/full_train/mean"
...
def _make_dataset(
    ...
    def _map(path: tf.Tensor, label: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
        return _decode_image(path, image_size=image_size, training=training), label
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the isolated RGNet-paper AVA binary classification model.")
    ...
    parser.add_argument("--score_activation_mode")
...
def main() -> None:
    ...
    batch_size = int(_resolve(args.batch_size, config, "training", "batch_size", 16))
    ...
    backbone = str(_resolve(args.backbone, config, "model", "backbone", "DenseNet121"))
    ...
    aggregation = str(_resolve(args.aggregation, config, "model", "aggregation", "mean")).lower()
    lse_r = float(_resolve(args.lse_r, config, "model", "lse_r", 4.0))
    score_activation_mode = str(_resolve(args.score_activation_mode, config, "model", "score_activation_mode", RAW_THEN_SIGMOID))
    ...
    train_frame = _load_frame(str(train_csv), image_col, score_col, image_dir, label_threshold, max_train_samples)
    val_frame = _load_frame(str(val_csv), image_col, score_col, image_dir, label_threshold, max_val_samples)
    ...
    train_ds = _make_dataset(train_frame, image_size, batch_size, training=True, shuffle=True)
    val_ds = _make_dataset(val_frame, image_size, batch_size, training=False, shuffle=False)

    model = build_rgnet_paper_ava_model(
    ...
    summary = {
        ...
        "paper_comparability_note": (
            ...
            "residual graph convolution, region-level logits, and configurable aggregation. "
        ...
        "model": {
            ...
            "region_classification_head": "per-node scalar logits",
            "aggregation": aggregation,
            "lse_r": lse_r,
            "score_activation_mode": score_activation_mode,
            "aggregation_formula": "(1 / r) * log(mean(exp(r * values))) for aggregation=lse",
        },
        "tensorflow": tf_info,
