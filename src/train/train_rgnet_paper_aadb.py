...
from src.models.rgnet_paper import (
    MODEL_VARIANT,
    build_rgnet_paper_model,
    get_rgnet_paper_custom_objects,
)


DEFAULT_OUTPUT_DIR = "outputs/rgnet_paper_aadb_regression_20260509/full_train"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the isolated RGNet-paper-v0 AADB regression model.")
...
def main() -> None:
    ...
    patience = int(_resolve(args.patience, config, "training", "patience", 3))
    ...
    graph_blocks = int(_resolve(args.graph_blocks, config, "model", "graph_blocks", 2))
    ...
    if verify_save_load:
        loaded = tf.keras.models.load_model(
            ...
            compile=False,
            safe_mode=False,
            custom_objects=get_rgnet_paper_custom_objects(),
        )
    ...
    summary = {
        ...
        "paper_comparability_note": (
            "Uses DenseNet121 and region graph reasoning, but region nodes are spatial feature-map "
            "positions with cosine-similarity adjacency; this is not the official paper implementation."
        ),
